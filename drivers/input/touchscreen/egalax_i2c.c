...
static SIMPLE_DEV_PM_OPS(egalax_i2c_pm, pm_suspend, pm_resume);
...
static int __devinit egalax_i2c_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	{
		p_egalax_i2c_dev->reset_gpio = of_get_named_gpio(devnode, "rst-gpio", 0);
		p_egalax_i2c_dev->interrupt_gpio = of_get_named_gpio(devnode, "intr-gpio", 0);
	}
#else
	{
	...
	ret = gpio_request(p_egalax_i2c_dev->reset_gpio, "Touch Reset");
	if (ret < 0)
	{
		EGALAX_DBG(DBG_MODULE, " gpio_request[%d] failed: %d, for Touch Reset\n", p_egalax_i2c_dev->reset_gpio, ret);
		if (ret != -EBUSY)
			goto fail1;
	}
	...
	gpio_free(p_egalax_i2c_dev->reset_gpio);
	...
	ret = request_irq(client->irq, egalax_i2c_interrupt, IRQF_TRIGGER_LOW | IRQF_ONESHOT, client->name, p_egalax_i2c_dev);
	if( ret ) 
	{
