							...
							struct pn544_i2c_platform_data *pdata)
{
	...
	val = of_get_named_gpio_flags(node, "irq-gpios", 0, &flags);
...
#endif
{
	...
	printk("%s: request irq_gpio %d\n", __func__, pdata->irq_gpio);
	ret = gpio_request(pdata->irq_gpio, "nfc_int");
	if (ret){
		printk("%s :not able to get GPIO irq_gpio\n", __func__);
		return  -ENODEV;
	}
	ret = gpio_to_irq(pdata->irq_gpio);
	if (ret < 0){
		printk("%s :not able to map GPIO irq_gpio to an IRQ\n", __func__);
		goto err_ven;
	}
	else{
		client->irq = ret;
...
printk("failed to allocate memory for module data\n");
	...
	if (ret < 0) {
		printk("%s :not able to set irq_gpio as input\n", __func__);
		goto err_exit;
	}

	ret = gpio_direction_output(pn54x_dev->ven_gpio, 0);
	if (ret < 0) {
		printk("%s : not able to set ven_gpio as output\n", __func__);
		goto err_exit;
	}
	...
	if (gpio_is_valid(pn54x_dev->firm_gpio)) {
		...
		if (ret < 0) {
			printk("%s : not able to set firm_gpio as output\n",
				 __func__);
			goto err_exit;
		}
	}
	...
	if (gpio_is_valid(pn54x_dev->clkreq_gpio)) {
		...
		if (ret < 0) {
			printk("%s : not able to set clkreq_gpio as output\n",
				   __func__);
			goto err_exit;
		}
	}
	...
	//ty: set to enable while probe done
//	pn544_enable(pn54x_dev, MODE_RUN);
	return 0;
