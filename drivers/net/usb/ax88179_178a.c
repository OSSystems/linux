...
static int __ax88179_write_cmd(struct ax88179 *dev, u8 cmd, u16 value, u16 index,
			       u16 size, void *data, int in_pm)
{
	int ret;
...
static int ax88179_tx_agg_fill(struct ax88179 *dev, struct tx_agg *agg)
{
	...
	while (remain >= ETH_ZLEN + 8) {
		...
		remain = agg_buf_sz_tx - (int)(tx_agg_align(tx_data) - agg->head);
	}
...
int ax88179_submit_rx(struct ax88179 *dev, struct rx_agg *agg, gfp_t mem_flags)
{
	...
	if (test_bit(AX88179_UNPLUG, &dev->flags) ||
	    !test_bit(WORK_ENABLE, &dev->flags) || !netif_carrier_ok(dev->netdev))
		return 0;
...
{	
	...
	while(time_before(jiffies, jtimeout)) {
		...
		ax88179_read_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_PHY_PHYSR, 2, &reg16, 1);
...
static int ax88179_system_resume(struct ax88179 *dev)
{
	...
	if (netif_running(netdev) && (netdev->flags & IFF_UP)) {
		...
		ax88179_write_cmd_nopm(dev, AX_ACCESS_MAC, AX_CLK_SELECT, 1, 1, &reg8);
		msleep(100);

		/* Configure RX control register => start operation */
		reg16 = AX_RX_CTL_DROPCRCERR | AX_RX_CTL_START | AX_RX_CTL_AP |
			 AX_RX_CTL_AMALL | AX_RX_CTL_AB;		
		ax88179_write_cmd_nopm(dev, AX_ACCESS_MAC, AX_RX_CTL, 2, 2, &reg16);
...
static int access_eeprom_mac(struct ax88179 *dev, u8 *buf, u8 offset, int wflag)
{
	...
	if (!wflag) {
		if (ret < 0) {
			netdev_dbg(dev->netdev, "Failed to read MAC address from EEPROM: %d\n", ret);
			return ret;
		}
...
static int ax88179_get_mac(struct ax88179 *dev, u8* buf)
{
	...
	if (ax88179_check_ether_addr(dev)) {
		...
		if (ret < 0) {
			netdev_err(dev->netdev, "Failed to write MAC to EEPROM: %d", ret);
			goto out;
		}
		...
		if (ret < 0) {
			netdev_err(dev->netdev, "Failed to read MAC address: %d", ret);
			goto out;
		}
...
static int ax88179_convert_old_led(struct ax88179 *dev, u8 efuse, void *ledvalue)
{
	u8 ledmode = 0;
	u16 reg16;
	u16 led = 0;	
...
static int ax88179_bind(struct ax88179 *dev)
{
	...
	reg8 = 0x34;
	ax88179_write_cmd(dev, AX_ACCESS_MAC, AX_PAUSE_WATERLVL_LOW, 1, 1, &reg8);
